...
derefptr(thisptr, vtidx) = pointer(Void, unsafe_ref(pointer(Uint64, unsafe_ref(pointer(Uint64,thisptr))))+vtidx)
...
macro vcall(vtidx, ret_type, func, arg_types)
  ...
  quote
    function $(esc(func))(thisptr, $(_args_in...))
      fptr = derefptr(thisptr, $(vtidx) )
      ccall( fptr, thiscall, $(esc(ret_type)), $arg_types, $(_args_in...) )
    end
  end
end
